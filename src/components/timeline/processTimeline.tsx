					...
					return (
						<li key={index} className='flex mx-4 sm:m-4  sm:flex-row flex-col '>
							<div className='hidden items-start w-48 pt-0.5 sm:flex text-brown'>
								<p className='w-4/5 text-right pr-2'>{process.name}</p>
							...
							<div
								className={`   w-full text-center z-20 sm:w-96  bg-${color}-600 rounded px-8 p-4 w-full overflow-hidden  text-white flex flex-col items-center`}
							>
								<img
									src={process.img}
									alt='icon'
									className={`bg-${color}-600 w-8 h-8 p-1 rounded z-20 sm:hidden`}
								/>
								...
								<ul className='list-disc text-left'>
									{process.steps.map((step, index) => {
										return (
											<li className=' text-base' key={`${process.name}-${index}`}>
												{step}
											</li>
										);
									})}
